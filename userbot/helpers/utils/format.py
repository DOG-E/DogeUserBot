...
async def paste_message(text, pastetype="p", extension=None, markdown=True, title=None):
    if markdown:
        text = md_to_text(text)
    if title or pastetype == "t":
        response = await pastetext(text, pastetype, extension, title)
        return response
    else:
...
# Credits: Uniborg @spechide - https://github.com/SpEcHiDe/UniBorg/blob/d8b852ee9c29315a53fb27055e54df90d0197f0b/uniborg/utils.py#L250
...
def yaml_format(obj, indent=0, max_str_len=256, max_byte_len=64):  # sourcery no-metrics
    """
