...
from http.client import _MAXHEADERS, BadStatusLine, IncompleteRead
...
def user_input():
    ...
    if object_check["config_file"] != "":
        json_file = load(open(config_file_check[0].config_file))
...
class googleimagesdownload:
    ...
    def _extract_data_pack_ajax(self, data):
        lines = data.split("\n")
        return loads(lines[3])[0][2]

    def _image_objects_from_pack(self, data):
        image_objects = loads(data)[31][-1][12][2]
    ...
    def similar_images(self, similar_images):
        try:
            ...
            req1 = Request(searchUrl, headers=headers)
            resp1 = urlopen(req1)
            ...
            req2 = Request(newurl, headers=headers)
            urlopen(req2)
    ...
    def keywords_from_file(self, file_name):
        search_keyword = []
        with codecopen(file_name, "r", encoding="utf-8-sig") as f:
    ...
    def create_directories(self, main_directory, dir_name, thumbnail, thumbnail_only):
        ...
        try:
            if not ospath.exists(main_directory):
                makedirs(main_directory)
                sleep(0.15)
            path = dir_name
            sub_directory = ospath.join(main_directory, path)
            if not ospath.exists(sub_directory):
                makedirs(sub_directory)
            if thumbnail or thumbnail_only:
                sub_directory_thumbnail = ospath.join(
                    main_directory, dir_name_thumbnail
                )
                if not ospath.exists(sub_directory_thumbnail):
                    makedirs(sub_directory_thumbnail)
        except OSError as e:
            if e.errno != 17:
                raise
        return
            ...
            try:
                ...
                try:
                    ...
                    if save_source:
                        ...
                        list_file.write(path + "\t" + img_src + "\n")
                        list_file.close()
                    absolute_path = ospath.abspath(path)
                except OSError as e:
                    download_status = "fail"
                    download_message = (
                        "OSError on an image...trying next one..." + " Error: " + str(e)
    ...
    def download(self, arguments):
        ...
        if __name__ == "__main__":
            ...
            records = []
            json_file = load(open(arguments["config_file"]))
            for record in range(len(json_file["Records"])):
                arguments = {}
                for i in args_list:
                    arguments[i] = None
                ...
                for i in range(len(search_keyword)):  # 3.for every main keyword
                    ...
                    if arguments["extract_metadata"]:
                        ...
                        with open(
                            ...
                            dump(items, json_file, indent=4, sort_keys=True)
                    # Related images
