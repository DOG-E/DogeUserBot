...
async def very(event):
    ...
    LOGO_FONT = (
        ...
        or f"https://github.com/DOG-E/Source/blob/DOGE/Material/Logo/Fonts/Streamster.ttf?raw=true"
    )
    ...
    if cmd == "":
        ...
        await clippyart(event.client, file_name, event.chat_id, reply_to_id)
...
@doge.ub(
    ...
    info={
        ...
        "description": "Customize logo font, font size, font position like text hight or width.",
        "flags": {
...
async def cufont(event):
    "To customize logo font"
    cmd = event.pattern_match.group(1).lower()
    input_str = event.pattern_match.group(2)
    if cmd == "":
        source = requests.get(
        ...
        if input_str not in logo_font:
            ...
            string = f"https://github.com/DOG-E/Source/blob/DOGE/Material/Logo/Fonts/{input_str}.ttf?raw=true"
...
async def logo_generate(event):
    xx = await edit_or_reply(event, "`Proccessing...`")
    name = event.pattern_match.group(1)
    if not name:
        await edit_or_reply(xx, "`Give a name too!`")
    bg_, font_ = "", ""
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("userbot/helpers/styles/otherfonts/*")
    ...
    if os.path.exists(flnme):
        await event.client.send_file(
            event.chat_id,
            file=flnme,
            caption=f"Logo by [{ALIVE_NAME}](tg://user?id={mention})",
            force_document=True,
        )
        os.remove(flnme)
        await xx.delete()
    ...
    if os.path.exists(font_):
        if not font_.startswith("userbot/helpers/styles/otherfonts"):
            os.remove(font_)
