...
from . import BOTLOG, BOTLOG_CHATID, doge, edl, eor, fsmessage, gvar, newmsgres, tr
    ...
    if isinstance(info, list) is False:
        ...
        try:
            async with doge.conversation(chat) as conv:
                await fsmessage(event, songinfo, chat=chat)
                musics = await newmsgres(conv, chat)
                ...
                if musics.audio:
                    ...
                    songgg = await newmsgres(conv, chat)
                    await event.client.send_read_acknowledge(conv.chat_id)
                    await event.client.send_message(
