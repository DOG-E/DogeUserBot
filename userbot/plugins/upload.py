...
async def dogelst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        dogepath = os.path.join(path, file)
        if os.path.isfile(dogepath):
            dogesort.append(file)
    for file in contents:
        dogepath = os.path.join(path, file)
        if os.path.isdir(dogepath):
            dogesort.append(file)
    return dogesort
...
async def upload(path, event, udir_event, dogeflag=None):  # sourcery no-metrics
    dogeflag = dogeflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            dogepath = os.path.join(path, file)
            await upload(Path(dogepath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, dogeflag=flag)
        end = datetime.now()
