...
thumb_loc = osp.join(TMP_DOWNLOAD_DIRECTORY, "thumb_image.jpg")
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    final = osp.join(TEMP_DIR, "output.gif")
    ...
    for track in media_info.tracks:
        if track.track_type == "Video":
            ...
            height = track.height
            width = track.width
    PATH = osp.join(TEMP_DIR, "round.gif")
    if aspect_ratio != 1:
...
async def _(event):
    ...
    outputfile = osp.join(TEMP_DIR, "vidtogif.gif")
