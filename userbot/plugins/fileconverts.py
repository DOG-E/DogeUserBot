...
async def video_dogefile(event):  # sourcery no-metrics
    ...
    dogefile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not dogefile.endswith((".webp")):
            if dogefile.endswith((".tgs")):
                hmm = await make_gif(dogevent, dogefile)
                os.rename(hmm, "./temp/circle.mp4")
                dogefile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(dogefile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = width if (height > width) else height
                await _dogeutils.runcmd(
                    f'ffmpeg -i {dogefile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(dogefile, PATH)
            if str(dogefile) != str(PATH):
                os.remove(dogefile)
            try:
                dogethumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            dogethumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(dogefile, dogethumb)
        if dogethumb is not None and not os.path.exists(dogethumb):
            dogethumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, dogethumb)
        if (
            dogethumb is not None
            and not os.path.exists(dogethumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            dogethumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, dogethumb)
        if dogethumb is not None and os.path.exists(dogethumb):
            await _dogeutils.runcmd(
                f"""ffmpeg -loop 1 -i {dogethumb} -i {dogefile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(dogefile)
        else:
            os.remove(dogefile)
    ...
    if (
        ...
        and not dogefile.endswith((".webp"))
    ):
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=dogeid)
    await dogevent.delete()
...
async def on_file_to_photo(event):
    ...
    doget = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "Use quality of range 0 to 721")
    dogereply = await event.get_reply_message()
    dog_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not dogereply or not dogereply.media or not dogereply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if dogereply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    ...
    try:
        ...
        pass
    reply_to_id = await reply_id(event)
    dogefile = await event.client.download_media(dogereply)
    dogegif = await make_gif(event, dogefile, quality, fps)
    teledoge = await event.client.send_file(
        event.chat_id,
        dogegif,
