...
async def _(event):
    ...
    dogename, stderr = (await _dogeutils.runcmd(name_cmd))[:2]
    if stderr:
        return await dogevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    dogename = os.path.splitext(dogename)[0]
    # if stderr:
    #    return await dogevent.edit(f"**Error :** `{stderr}`")
    song_file = Path(f"{dogename}.mp3")
    if not os.path.exists(song_file):
        return await dogevent.edit(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await dogevent.edit("`yeah..! i found something wi8..🥰`")
    dogethumb = Path(f"{dogename}.jpg")
    if not os.path.exists(dogethumb):
        dogethumb = Path(f"{dogename}.webp")
    elif not os.path.exists(dogethumb):
        dogethumb = None
    ...
    for files in (dogethumb, song_file):
...
async def _(event):
    ...
    #    return await dogevent.edit(f"**Error :** `{stderr}`")
    dogename = os.path.splitext(dogename)[0]
    vsong_file = Path(f"{dogename}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{dogename}.mkv")
    ...
    await dogevent.delete()
    for files in (dogethumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
