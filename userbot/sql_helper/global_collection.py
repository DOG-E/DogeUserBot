...
class Doge_GlobalCollection(BASE):
    __tablename__ = "doge_globalcollection"
    ...
    def __repr__(self):
        return "<Doge Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Doge_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Doge_GlobalCollection.__table__.create(checkfirst=True)

DOGE_GLOBALCOLLECTION = RLock()
...
def add_to_collectionlist(keywoard, contents):
    with DOGE_GLOBALCOLLECTION:
        keyword_items = Doge_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with DOGE_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Doge_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with DOGE_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Doge_GlobalCollection.keywoard)
            .filter(Doge_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Doge_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Doge_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Doge_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Doge_GlobalCollection).all()
        for x in all_groups:
