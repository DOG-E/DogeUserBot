...
from dogebot import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"dogebot/plugins/{shortname}.py")
        checkplugins(path)
        name = "dogebot.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"dogebot/plugins/{shortname}.py")
            name = f"dogebot.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["dogebot.plugins." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
