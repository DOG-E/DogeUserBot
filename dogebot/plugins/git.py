...
@doge.ub(
    ...
    info={
        ...
        "description": "It uploads the given file to your github repo in **dogebot/plugins** folder\
...
async def git_commit(file_name, mone):
    ...
    if create_file:
        file_name = "dogebot/plugins/" + file_name
        LOGS.info(file_name)
        try:
            repo.create_file(
                file_name, "Uploaded New Plugin", commit_data, branch="DOGE"
            )
            LOGS.info("Committed File")
            ccess = Config.GIT_REPO_NAME
            ccess = ccess.strip()
            await mone.edit(
                f"`Commited On Your Github Repo`\n\n[Your PLUGINS](https://github.com/{ccess}/tree/DOGE/dogebot/plugins/)"
            )
