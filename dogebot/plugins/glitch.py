...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    dogeinput = int(dogeinput) if dogeinput else 2
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, dogeinput, color_offset=True)
        glitch_img.save(glitched)
        await event.client.send_file(event.chat_id, glitched, reply_to=dogeid)
    else:
        glitched = os.path.join("./temp", "glitched.gif")
        glitch_img = glitcher.glitch_image(img, dogeinput, color_offset=True, gif=True)
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
            glitched,
