...
if Config.ANTISPAMBOT_BAN:
    ...
    async def anti_spambot(event):  # sourcery no-metrics
        ...
        dogeadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not dogeadmin:
            return
        dogebanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            dogegban = get_gbanuser(user.id)
            if dogegban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{dogegban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not dogebanned:
