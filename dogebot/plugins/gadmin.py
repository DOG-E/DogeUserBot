...
async def dogegban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    dogee = await edit_or_reply(event, "`gbanning.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, dogee)
    if not user:
        return
    if user.id == doge.uid:
        return await edit_delete(dogee, "`why would I ban myself`")
    if gban_sql.is_gbanned(user.id):
        await dogee.edit(
            f"`the `[user](tg://user?id={user.id})` is already in gbanned list any way checking again`"
        )
    else:
        gban_sql.dogegban(user.id, reason)
    san = await admin_groups(event.client)
    count = 0
    teledoge = len(san)
    if teledoge == 0:
        return await edit_delete(dogee, "`you are not admin of atleast one group` ")
    await dogee.edit(
    ...
    dogetaken = (end - start).seconds
    if reason:
        await dogee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {dogetaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await dogee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {dogetaken} seconds`!!"
...
async def dogegban(event):
    "To unban the person from every group where you are admin."
    dogee = await edit_or_reply(event, "`ungbanning.....`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, dogee)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.dogeungban(user.id)
    else:
        return await edit_delete(
            dogee, f"the [user](tg://user?id={user.id}) `is not in your gbanned list`"
        )
    san = await admin_groups(event.client)
    count = 0
    teledoge = len(san)
    ...
    if reason:
        await dogee.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {dogetaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await dogee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {dogetaken} seconds`!!"
...
async def dogegkick(event):  # sourcery no-metrics
    "kicks the person in all groups where you are admin"
    dogee = await edit_or_reply(event, "`gkicking.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, dogee)
    if not user:
        return
    if user.id == doge.uid:
        return await edit_delete(dogee, "`why would I kick myself`")
    san = await admin_groups(event.client)
    count = 0
    teledoge = len(san)
    if teledoge == 0:
    ...
    if reason:
        await dogee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {dogetaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await dogee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {dogetaken} seconds`!!"
