...
def verify_cond(dogearray, text):
    return any(i in text for i in dogearray)
...
def char_is_emoji(character):
    return character in dogemoji.UNICODE_EMOJI["en"]
...
def pack_nick(username, pack, is_anim):
    if gvarstatus("CUSTOM_STICKER_PACKNAME"):
        ...
        packnick = f"@{username} {pack} @DogeUserBot Anim"
    else:
        packnick = f"@{username} {pack} @DogeUserBot"
    return packnick
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await dogevent.edit("check `.info stickers`")
        ...
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, dogepackname = await newpacksticker(
                        dogevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, dogepackname = await add_to_pack(
                        dogevent,
                        conv,
                        event,
                        packname,
            ...
            if dogepackname not in blablapacks:
                blablapacks.append(dogepackname)
