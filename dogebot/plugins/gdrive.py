...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        if dogetorrent:
            ...
            await edit_or_reply(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @dogeplugins",
            )
...
async def g_download(event):
    ...
    file_name, dogeprocess = await gdrive_download(event, dogevent, service, drive_link)
    if dogeprocess is not None:
