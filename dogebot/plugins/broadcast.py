...
@doge.ub(
    ...
    info={
        ...
        "examples": "{tr}msgto @DogeUserBotot just a testmessage",
    },
)
...
async def dogebroadcast_add(event):
    ...
    dogeinput_str = event.pattern_match.group(1)
    if not dogeinput_str:
        return await edit_delete(
            event,
            "In which category should i add this chat",
            parse_mode=_format.parse_pre,
        )
    keyword = dogeinput_str.lower()
...
async def dogebroadcast_delete(event):
    ...
    dogeinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(dogeinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {dogeinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(dogeinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {dogeinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
