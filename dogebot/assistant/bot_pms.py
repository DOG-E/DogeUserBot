...
from dogebot import Config, doge
...
async def bot_start(event):
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"🐶 Hey! {mention},\
                        \n🐾 I am {my_mention}'s helper dog.\
                        \nℹ You can contact to my master from here.\
                        \n\n🐾 Powered by @DogeUserBot"
        buttons = [
            (
    ...
    try:
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                f"**❗ ERROR:**\nℹ There was a error while user starting your bot.\
                \n➡ `{str(e)}`",
            )
    else:
    ...
    if chat.id != Config.OWNER_ID:
        ...
        try:
            ...
            if BOTLOG:
                await event.client.send_message(
                    BOTLOG_CHATID,
                    f"**❗ ERROR:**\nℹ While storing messages details in database.\n➡ `{str(e)}`",
                )
    else:
...
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
        return await event.reply("ℹ Reply to a message to get message info!")
    ...
    if users is None:
        return await info_msg.edit(
            "**❗ ERROR:** \nℹ `Sorry!, Can't Find this user in my database :(`"
        )
    for usr in users:
        user_id = int(usr.chat_id)
        user_name = usr.first_name
...
async def send_flood_alert(user_) -> None:
    ...
    flood_msg = (
        ...
        "\n\n"
        f"  **🆔 ID:** `{user_.id}`\n"
        f"  **ℹ Name:** {get_display_name(user_)}\n"
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                sudo_spam = (
                    ...
                    "ℹ Is Flooding your bot !, Check `.help delsudo` to remove the user from Sudo."
                )
                if BOTLOG:
        ...
        try:
            ...
            if BOTLOG:
                await doge.tgbot.send_message(BOTLOG_CHATID, "🛡 **Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@doge.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
...
@doge.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
